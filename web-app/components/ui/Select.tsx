...
export function Select({
	...
	return (
		<div className="w-full">
			{label && <label className={cn("mb-1 block text-sm font-medium text-gray-900", classNameLabel)}>{label}</label>}
			...
			<div className={cn("relative", className)}>
				<ReactSelect
					...
					noOptionsMessage={() => <span className="text-sm text-gray-400">Aucune option disponible</span>}
					classNames={{
						control: ({ isFocused }) =>
							cn(
						...
						multiValue: () => "bg-sky-100 text-sky-800 rounded px-2 py-0.5 text-xs font-medium",
						multiValueRemove: () => "text-sky-600 hover:text-sky-800 ml-1 cursor-pointer",
				...
				{hasError && (
					<div className="absolute right-3 top-1/2 z-10 -translate-y-1/2 text-red-600">
						<AlertCircle size={16} />
					</div>
				)}
			</div>
