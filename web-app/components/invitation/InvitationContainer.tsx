...
export default function InvitationContainer({ token }: { token: UUID }) {
	...
	const { data, isPending } = useInvitationByToken(token);
			...
			<div className="flex w-full flex-col gap-10">
				...
				<div>
					...
					<p className="mb-2 flex flex-wrap items-baseline gap-2 text-gray-500">
						<span className="shrink-0">{isPending ? <Skeleton className="h-4 w-32 rounded-sm" /> : inviter}</span>
						<span className="whitespace-normal">vous a invité à rejoindre l&apos;entreprise</span>
						<span className="shrink-0">{isPending ? <Skeleton className="h-4 w-36 rounded-sm" /> : company}</span>
					</p>

					<p className="flex flex-wrap items-baseline gap-2 text-sky-600">
						<span>Rappel de votre adresse email :</span>
						<span className="shrink-0">{isPending ? <Skeleton className="h-4 w-40 rounded-sm" /> : email}</span>
					</p>
				</div>
