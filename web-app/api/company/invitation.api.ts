...
export function useInvitationByToken(token: UUID) {
	return useQuery({
		...
		queryFn: async () => {
			const res = await serviceClientNonAuthentifie.get<InvitationByTokenResponse>(`/public/invitations/${token}`);
			return res.data;
		},
		enabled: !!token,
	});
...
export function useInvitations(companyId: string) {
	return useQuery({
		...
		queryFn: async () => {
			const res = await serviceClient.get<InvitationResponse[]>(`/companies/${companyId}/administrators/invitations`);
			return res.data;
		},
		enabled: !!companyId,
	});
