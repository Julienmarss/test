	...
	return useMutation({
		mutationFn: async ({
			...
			const response = await serviceClient.post<CollaboratorResponse>(
				`/companies/${companyId}/collaborators/${collaboratorId}/documents`,
				formData,
				{
					headers: {
	...
	return useMutation({
		mutationFn: async ({
			...
			const response = await serviceClient.patch<CollaboratorResponse>(
				`/companies/${companyId}/collaborators/${collaboratorId}/documents/${document.id}`,
				document,
			);
			return response.data;
		},
	...
	return useMutation({
		mutationFn: async ({
			...
			const response = await serviceClient.delete<CollaboratorResponse>(
				`/companies/${companyId}/collaborators/${collaboratorId}/documents/${documentId}`,
			);
			return response.data;
		},
		onSuccess: (collaborator: CollaboratorResponse) => {
...
export function useVisualizeDocument() {
	return useMutation({
		mutationFn: async ({
			...
			const response = await serviceClient.get<CollaboratorDocumentResponse>(
				`/companies/${companyId}/collaborators/${collaboratorId}/documents/${documentId}`,
			);
			return response.data;
		},
