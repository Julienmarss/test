...
export function useAddNote() {
    ...
    return useMutation({
        mutationFn: async ({collaboratorId, companyId, content, title}: {
            collaboratorId: UUID,
            companyId: UUID,
            title: string,
            content: string
        }) => {
            const response = await serviceClient.post<CollaboratorResponse>(`/companies/${companyId}/collaborators/${collaboratorId}/notes`, {
                title: title,
                content: content
            });
...
export function useDeleteNote() {
    ...
    return useMutation({
        mutationFn: async ({collaboratorId, companyId, noteId}: {
            collaboratorId: UUID,
            companyId: UUID,
            noteId: UUID
        }) => {
            const response = await serviceClient.delete<CollaboratorResponse>(`/companies/${companyId}/collaborators/${collaboratorId}/notes/${noteId}`);
            return response.data;
        },
        onSuccess: (collaborator: CollaboratorResponse) => {
            queryClient.setQueryData(["collaborator", collaborator.id], () => collaborator);
