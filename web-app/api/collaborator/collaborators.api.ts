...
export function useCollaborator(companyId: string, collaboratorId: string) {
	return useQuery({
		...
		queryFn: async () => {
			const response = await serviceClient.get<CollaboratorResponse>(
...
export function useCollaboratorByToken() {
	return useMutation({
		mutationFn: async ({ token }: { token: string }) => {
			const response = await axios.post<CollaboratorResponse>(
				`${process.env.NEXT_PUBLIC_SERVICE_URL}/public/collaborator`,
				{
					token,
				},
			);
...
export function useImportCollaborators() {
	return useMutation({
		mutationFn: async ({ files, companyId }: { files: File[]; companyId: UUID }) => {
			...
			const response = await serviceClient.post<VoidFunction>(
				`/companies/${companyId}/collaborators/import`,
				formData,
				{
					headers: {
	...
	return useMutation({
		mutationFn: async ({ collaborator, companyId }: { collaborator: UpdateCollaboratorRequest; companyId: UUID }) => {
			const response = await serviceClient.patch<CollaboratorResponse>(
				`/companies/${companyId}/collaborators/${collaborator.id}`,
				collaborator,
			);
			return response.data;
		},
...
export function useUpdateCollaboratorByToken() {
	return useMutation({
		mutationFn: async ({ collaborator, token }: { collaborator: UpdateCollaboratorRequest; token: string }) => {
			const response = await axios.post<CollaboratorResponse>(
				`${process.env.NEXT_PUBLIC_SERVICE_URL}/public/collaborator/update`,
				{
					token,
					...collaborator,
				},
	...
	return useMutation({
		mutationFn: async ({ companyId, collaboratorId, file }: { companyId: UUID; collaboratorId: UUID; file: File }) => {
			...
			const response = await serviceClient.post(
				`/companies/${companyId}/collaborators/${collaboratorId}/picture`,
				formData,
				{
					headers: {
						"Content-Type": "multipart/form-data",
...
export function useModifyCollaboratorPictureByToken() {
	return useMutation({
		mutationFn: async ({ token, file }: { token: string; file: File }) => {
			...
			const response = await axios.post<CollaboratorResponse>(
				`${process.env.NEXT_PUBLIC_SERVICE_URL}/public/collaborator/picture`,
				formData,
				{
					headers: {
						"Content-Type": "multipart/form-data",
...
export function useSendMailCollaboratorToComplete() {
	return useMutation({
		mutationFn: async ({ companyId, collaboratorId }: { companyId: UUID; collaboratorId: UUID }) => {
			const response = await serviceClient.post(
				`/companies/${companyId}/collaborators/${collaboratorId}/fill-profile-request`,
			);
			return response.data;
		},
		onSuccess: () => {
