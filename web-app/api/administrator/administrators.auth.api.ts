...
export function useForgottenPassword() {
	return useMutation({
		mutationFn: async ({ email }: { email: string }) => {
			const response = await serviceClientNonAuthentifie.post<ReinitialisationMotDePasseResponse>(
				`/public/authentification/forgotten-password`,
				{ email },
			);
...
export function useModifyPassword() {
	return useMutation({
		mutationFn: async ({ email, password, token }: { email: string; password: string; token: string }) => {
			const response = await serviceClientNonAuthentifie.put<ReinitialisationMotDePasseResponse>(
				`/public/authentification/modify-password`,
				{
					email,
					token,
					password,
