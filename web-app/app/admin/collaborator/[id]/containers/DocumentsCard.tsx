...
const types: { name: string, value?: DocumentTypeResponse }[] = [{
    name: "Contrats et avenants", value: "Contrats et avenants"
}, {
    name: "Administratif", value: "Administratif"
}, {
    name: "Autres", value: "Autres"
}, {
    name: "Sans type", value: undefined
}];
                ...
                {types.map((type) => (
                    <div key={type.name}>
                        {documentsOfType(type.value).length > 0 &&
                            <span className="text-sm text-sky-600">{type.name}</span>}
                        <table className="w-max md:w-full overflow-x-auto">
                            <tbody>
                            {documentsOfType(type.value).map((document, index) => (
                                <tr key={index}>
                                    ...
                                    {editionMode && (
                                        <td className="align-middle p-2 min-w-[15em]">
                                            <Select
