...
const AddPictureProfile: React.FC<AddPictureProfileProps> = ({
	...
	const {
		mutate: modifyPicture,
		isPending: modifyPicturePending,
		isSuccess,
		data,
	} = useModifyCollaboratorPictureByToken();
	...
	const validateFile = (file: File): string | null => {
		...
		if (!acceptedFileTypes.includes(file.type)) {
			return `Type de fichier non supporté. Formats acceptés : ${acceptedFileTypes
				.map((type) => {
	...
	return (
		<div id="picture-profile" className="flex flex-col gap-y-4 rounded-lg border border-slate-200 bg-white p-2 md:p-6">
				...
				<div className="space-y-4">
					{uploadState.file && uploadState.preview ? (
						...
						<>
							...
							<div>
								<h3 className="semi-bold mb-2 text-base text-gray-900">Importez votre photo de profil</h3>
								<p className="text-sm text-gray-500">{getFileExtensions()}</p>
							</div>
						</>
					)}
					...
					<Button
						...
						className="semi-bold border border-gray-200 bg-white text-sm text-black hover:bg-gray-200"
					>
						<svg width="14" height="11" viewBox="0 0 14 11" fill="none" xmlns="http://www.w3.org/2000/svg">
							<path
								fill-rule="evenodd"
								clip-rule="evenodd"
								d="M3.5 11C1.567 11 0 9.433 0 7.5C0 6.06836 0.859559 4.83748 2.09075 4.29529C2.03138 4.03979 2 3.77356 2 3.5C2 1.567 3.567 0 5.5 0C6.94462 0 8.18482 0.875211 8.71929 2.12432C8.96493 2.04364 9.22737 2 9.5 2C10.8807 2 12 3.11929 12 4.5C12 4.72217 11.971 4.93756 11.9166 5.14262C13.1252 5.53 14 6.66283 14 8C14 9.65685 12.6569 11 11 11H3.5ZM4.21967 5.96967C3.92678 6.26256 3.92678 6.73744 4.21967 7.03033C4.51256 7.32322 4.98744 7.32322 5.28033 7.03033L6.25 6.06066L6.25 8.75C6.25 9.16421 6.58579 9.5 7 9.5C7.41421 9.5 7.75 9.16421 7.75 8.75L7.75 6.06066L8.71967 7.03033C9.01256 7.32322 9.48744 7.32322 9.78033 7.03033C10.0732 6.73744 10.0732 6.26256 9.78033 5.96967L7.53033 3.71967C7.23744 3.42678 6.76256 3.42678 6.46967 3.71967L4.21967 5.96967Z"
								fill="#0F172A"
							/>
						</svg>
