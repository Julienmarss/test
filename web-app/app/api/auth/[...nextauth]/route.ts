    ...
    callbacks: {
        async signIn({user, account}: { user: User; account: Account; profile: Profile }) {
            if (account.provider === "google" || account.provider === "azure-ad") {
                const tenant = account.provider === "google" ? "GOOGLE" : account.provider === "azure-ad" ? "MICROSOFT" : "LEGIPILOT";

                try {
                    const response = await
                        serviceClientNonAuthentifie
                            .post("/public/oauth-signin", {username: user.email});
                    if (!response.data) {
