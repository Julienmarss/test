...
export default function AdministrationDashboard() {
    ...
    return (
        <article className="bg-sky-50">
            ...
            <div className="overflow-x-auto p-8 bg-gray-50 min-h-[89vh]">
                <h1 className="text-2xl text-gray-900 my-5">Administration</h1>

                <section className="flex items-center justify-between pt-4 mb-4 border-t border-gray-200">
                    <p className="text-md text-sky-600">
                        Il y a actuellement {administrators?.length} administrateur(s) sur la plateforme.
                    </p>
                    <OutlineButton onClick={() => exportAdministrators()}>
                        Exporter
                    </OutlineButton>
                </section>

                <table className="w-full">
                    <thead className="border border-gray-100 rounded-t-lg">
                    <tr>
                        <th className="px-6 py-3 text-left text-xs font-bold">
                            Prénom Nom
                        </th>
                        <th className="px-6 py-3 text-left text-xs font-bold text-gray-900 tracking-wider">
                            Email
                        </th>
                        <th className="px-6 py-3 text-left text-xs font-bold text-gray-900 tracking-wider">
                            Téléphone
                    ...
                    {administrators?.map((collaborator) => (
                        <tr key={collaborator.id}>
                            <td className="px-6 py-4 whitespace-nowrap">
                                <div
                                    className="text-sm text-gray-900">{collaborator.firstname} {collaborator.lastname}</div>
                            </td>
                            <td className="px-6 py-4 whitespace-nowrap">
                            ...
                            <td className="px-6 py-4 whitespace-nowrap">
                                <div
                                    className="text-sm text-gray-900">{collaborator.companies.map((company) => company.name).join(', ')}</div>
                            </td>
