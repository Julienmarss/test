...
export const MyCollaboratorsTrombinoscope = ({
	...
	return (
		<div className="grid grid-cols-1 gap-4 overflow-x-auto bg-gray-50 px-2 py-10 md:grid-cols-3 md:px-14">
			{collaborators.length > 0 &&
				collaborators.map((collaborator) => (
					<div key={collaborator.id} className="flex flex-col gap-x-4 rounded-xl border border-gray-200 bg-white p-4">
						<section className="flex w-full items-center justify-between md:items-start">
							<Checkbox
						...
						<div
							className="mb-4 ml-4 flex w-full cursor-pointer flex-row gap-4"
						...
						>
							...
							>
								{collaborator.picture && collaborator.picture.length > 0 ? (
									<img
										...
										className={`h-24 w-24 rounded-full bg-white ${getStatusBorderColor(collaborator.status)}`}
									/>
								) : (
									<div
										className={`flex h-24 w-24 items-center justify-center rounded-full border border-green-300 bg-blue-100 text-3xl font-medium text-blue-700`}
									>
							...
							<div className="flex flex-col items-start gap-2">
								<span className="flex items-center justify-center gap-2 text-gray-900 md:justify-start">
