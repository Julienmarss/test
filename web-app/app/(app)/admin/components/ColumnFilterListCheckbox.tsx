						...
						if (type === "status") {
							return (
								<div key={option.value} className="flex items-center justify-between">
									...
									<div
										className={`ml-4 flex h-4 w-4 items-center justify-center rounded-full border-4 ${getStatusColorCollaborator(option.value as StatusResponseCollaborator).border} `}
									>
										<div
											className={`h-2 w-2 rounded-full ${getStatusColorCollaborator(option.value as StatusResponseCollaborator).bg} `}
										/>
									</div>
								</div>
							);
