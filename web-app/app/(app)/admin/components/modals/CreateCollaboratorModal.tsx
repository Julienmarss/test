...
import { SituationContractuelleForm } from "@/app/(app)/admin/components/steps/SituationContractuelleForm";
import { CoordonneesForm } from "@/app/(app)/admin/components/steps/CoordonneesForm";
import { SituationPersonnelleForm } from "@/app/(app)/admin/components/steps/SituationPersonnelleForm";
import { DocumentsForm } from "@/app/(app)/admin/components/steps/DocumentsForm";
import { isServiceError } from "@/api/client.api";
import { INITIAL_CREATION_STATE } from "@/app/(app)/admin/components/modals/create-collaborator.service";
			...
			footer={
				<div className={`flex w-full flex-row ${currentStep > 0 ? "justify-between" : "justify-end"}`}>
					<>
						...
						{/* Messages d'erreur */}
						{isAddError && isServiceError(addError) && <span className="text-sm text-red-500">{addError.message}</span>}
